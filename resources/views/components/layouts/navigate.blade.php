            ...
            <a href="{{ route("cart.view") }}" class="">
                <div class="icon shopping-bag relative">
                    ...
                    <div class="bg-dark rounded-full text-light min-w-4 min-h-4 p-0.5 m-0 absolute top-2.5 left-2">
                        <p id="cartItemCount" class="p12 not-italic leading-3 text-center">{{Cart::instance('default')->count()}}</p> {{-- number of items in cart in real time --}}
                    </div>
                </div>
            </a>
        </li>
...
<nav id="main-nav" class="mb-6 px-4">
    <a href="{{ route("home.{$locale}") }}" class="logo">
        <img src="{{ asset('img/logo/lg/' . $generalInfo['logo_black']) }}" alt="{{ __('phrases.logo', ['name' => $generalInfo['company_name'], "de" => $locale == 'ca' ? (\App\Services\Translation\OrthographicRules::startsWithDe("de ". $generalInfo['company_name']) ? __('orthographicRules.with_d') : __('orthographicRules.with_de')) : __('orthographicRules.by')]) }}" style="width: 5em">
    </a>
    <ul class="nav-links">
