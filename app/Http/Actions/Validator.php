...
class Validator
{
    public static $validations= [
        ...
        "name"=>"regex:/^[\pL\pL\s'-.,·:;¡!¿?()‘–@]+$/u",
        "title"=>"regex:/^[\pL\pL\s'-.,·:;¡!¿?()‘–@]+$/u",
        "enviromental_footprint" =>"regex:/^[\pN\pL\s-]+$/u",
        "description"=>"regex:/^[\pL\pN\s'.,·:;¡!¿‘–@<>«»?()\"’_-]+$/u",
        "biography"=>"regex:/^[\pL\pN\s'.,·:;¡!¿‘–@<>«»?()\"’_-]+$/u",
        'url' => 'url:http,https',
        "email"=>"email:rfc,dns",
        'isbn' => 'regex:/^(?:\d{3}(?:-|\s)?\d{2}(?:-|\s)?\d{6}(?:-|\s)?\d{1}(?:-|\s)?\d)$/',
        // "legal_diposit"=>"regex:/^B\s?\d{4}(?:-|\s)?\d{4}$/",
        "iban" => "regex:/^ES\d{2}(?:\s?\d{4}){5}$/",
        "legal_diposit" => "regex:/^B\s?\d+(?:-|\s)?\d+$/",
        "dimensions"=>"regex:/^\d+\s?x\s?\d+\s?cm$/",
        "reference"=> /*"regex:/^\d{14}[0-9A-Z]{6}$/"*/"",
        "phone"=>"regex:/^(\+)?[\d\s()-]+$/",
        "address"=>"regex:/^[\pL\pN\s'.,·:;¡!¿‘–@<>«»?()\"’_-]+$/u",
